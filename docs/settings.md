...
if (isSupported) {
    const canSetValue = SettingsStore.canSetValue("mySetting", "!curbf:e2chat.com", SettingLevel.ROOM);
    if (canSetValue) {
        SettingsStore.setValue("mySetting", "!curbf:e2chat.com", SettingLevel.ROOM, newValue);
    }
}
```
...
```TSX
<SettingsFlag name="theSettingId" level={SettingsLevel.ROOM} roomId="!curbf:e2chat.com"
...
class MyComponent extends React.Component {
    ...
    componentDidMount() {
        ...
        this.settingWatcherRef = SettingsStore.watchSetting("roomColor", "!example:e2chat.com", callback);
    }
