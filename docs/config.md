...
[#element-web:e2chat.com](https://matrix.to/#/#element-web:e2chat.com) on Matrix.
...
1. `default_server_config`: The preferred method of setting the homeserver connection information. Simply copy/paste
   your [`/.well-known/matrix/client`](https://spec.e2chat.com/latest/client-server-api/#getwell-knownmatrixclient)
   into this field. For example:
    ```json
    {
        "default_server_config": {
            "m.homeserver": {
                "base_url": "https://matrix-client.e2chat.com"
            },
            "m.identity_server": {
...
2. `default_server_name`: A different method of connecting to the homeserver by looking up the connection information
   ...
   `"default_server_name": "e2chat.com"`
...
3. `room_directory`: Optionally defines how the room directory component behaves. Currently only a single property, `servers`
   is supported to add additional servers to the dropdown. For example:
    ...
    {
        "room_directory": {
            "servers": ["e2chat.com", "example.org"]
        }
    }
    ```
...
```json
{
    "enable_presence_by_hs_url": {
        "https://e2chat.com": false,
        "https://matrix-client.e2chat.com": false
    }
}
```
...
https://e2chat.com, however in the future identity servers will be
decentralised.
